...
<img src="https://i.imgur.com/dUtI19a.png" height=300>
...
Generally speaking, a quantum state is fragile. We have to deal with it to achieve successful quantum communication. Photon loss is one of them about which we need to care. Photon might be lost with some probability in the fiber with some possibility. If either photon from end nodes can't reach the BSA, then we can't apply BSA, which means, we can't generate Bell state between them. In the quisp, we can set a value of photon loss in the .ned file. In this experiment, we set 0.1 /km. 

### 1.3 Emission success probability

When a quantum node tries to emit a photon to the other nodes or components, that node might fail to emit it with some problems. (e.g., A node couldn't generate a Bell pair successfully.) We define a probability that a node fails to emit a photon as an "emission success probability." In this experiment, we set 0.9 for each quantum node.

### 1.4 Estimated success probability of a total experiment
...
![num_trials_plot](https://i.imgur.com/7ptQfmB.png)  
![num_trials_value](https://i.imgur.com/GmieQZu.png)  
We could get a value of 4.2556. This value is close to the ideal value of 4.286694. In terms of the average number of BSA, we could say quisp is working properly.
...
This is the result of the number of trials to generate one Bell pair.

![dist_bsa_trial](https://i.imgur.com/sRJTpdj.png) 

In addition to that, this is the result of the number of trials to generate a specific number of Bell pairs. As you can see, the distribution goes to Poisson distribution. 

![dist_trial_bsa_ridge](https://i.imgur.com/on4Gd8X.png)

### 4. Actual operation time for BSA

In the BSA process, we have a lot of operations. For example, exchanging packets, rulesets, applying gates, and so on.

We will see the time to complete this whole process.

In the simulation, we need 0.000009740086 s to complete the whole BSA process. So the average creation time of the Bell pair is 0.000009740086*4.2556 = 4.1449e-5 (s)

### 5. Distribution of the creation time
The distribution of the operation time to create one Bell pair is,

![dist_creation_time](https://i.imgur.com/xO7cGdb.png)

and also, a ridge plot for generating any number of Bell pairs is

![dist_creation_any](https://i.imgur.com/g3m1ZK4.png).
